...
function changeDirection(e) {
  ...
  if (e.code == "ArrowRight") {
    // check edges
    if (rocket0[0] >= cols - 2 - rocketHalf return;
    ...
    if (rocket0[0] <= 1 + rocketHalf return;
...
function update() {
  ...
  ctx.fillStyle = "red";
  ctx.fillRect(
    rocket0[0] * blockSize - (rocketHalf * blockSize,
    rocket0[1] * blockSize,
  ...
  ctx.fillStyle = "green";
  ctx.fillRect(
    rocket1[0] * blockSize - (rocketHalf * blockSize,
    rocket1[1] * blockSize,
    blockSize * rocketWidth,
    blockSize
  );
